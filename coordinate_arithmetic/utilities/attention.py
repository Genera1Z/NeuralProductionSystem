...
from .SharedGroupLinearLayer import SharedGroupLinearLayer, NonSharedGroupLinearLayer
...
class GroupMLP(nn.Module):
    def __init__(self, in_dim, out_dim, num_blocks, hidden_dim=32):
        super().__init__()
        self.mlp = nn.Sequential(GroupLinearLayer(in_dim, hidden_dim, num_blocks),
            nn.ReLU(),
            GroupLinearLayer(hidden_dim, hidden_dim, num_blocks),
            nn.ReLU(),
            GroupLinearLayer(hidden_dim, hidden_dim, num_blocks),
            nn.ReLU(),
            GroupLinearLayer(hidden_dim, out_dim, num_blocks))
...
class SharedGroupMLP(nn.Module):
    def __init__(self, in_dim, out_dim, num_blocks, hidden_dim=32):
        super().__init__()
        self.mlp = nn.Sequential(SharedGroupLinearLayer(in_dim, hidden_dim, num_blocks),
            nn.ReLU(),
            SharedGroupLinearLayer(hidden_dim, hidden_dim, num_blocks),
            nn.ReLU(),
            SharedGroupLinearLayer(hidden_dim, hidden_dim, num_blocks),
            nn.ReLU(),
            SharedGroupLinearLayer(hidden_dim, out_dim, num_blocks))
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, n_head, d_model_read, d_model_write, d_model_out, d_k, d_v, num_blocks_read, num_blocks_write,
            ...
            flag=False, schema_specific=False):
        super().__init__()
        ...
        if schema_specific:
            self.GLN_qs = NonSharedGroupLinearLayer(d_model_read, n_head * d_k, n_templates)
            self.GLN_ks = NonSharedGroupLinearLayer(d_model_write, n_head * d_k, n_templates)
            self.GLN_vs = NonSharedGroupLinearLayer(d_model_write, n_head * d_v, n_templates)
        elif share_inp:
    ...
    def forward(self, q, k, v, mask=None):
        ...
        residual = q
        if mask is not None:
            q = self.GLN_qs(q, mask).view(sz_b, len_q, n_head, d_k)
            # q = self.w_qs(q).view(sz_b, len_q, n_head, d_k)
            k = self.GLN_ks(k, mask).view(sz_b, len_k, n_head, d_k)
            v = self.GLN_vs(v, mask).view(sz_b, len_v, n_head, d_v)
