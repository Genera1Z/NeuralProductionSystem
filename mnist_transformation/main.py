...
from datum import get_dataloaders
...
def epoch_t(model, dataset, metric, optim, device, n_rule):
    ...
    for i, batch in tqdm(enumerate(dataset)):
        ...
        inputs_ = frames[:, :-1, ...].flatten(0, 1)[:, None, ...]
        ...
        outputs_ = outputs_[:, 0, ...]
        targets_ = frames[:, 1:, ...].flatten(0, 1)
        loss = metric(outputs_, targets_)

        loss.backward()
        nn.utils.clip_grad_norm_(model.parameters(), 1.0)  # XXX remove?
        optim.step()

        losses.append(loss.detach().cpu().item())  # loss.detach().cpu().item()
...
def main():
    ...
    dload_t, dload_v = get_dataloaders(n_operation, batch_size, '../data/', False, 4, None)
    model = MnistOperationNps(n_rule, c_rule, nv=2, cv=128).to(device)
    ...
    for n in range(n_epoch):
        ...
        print(f'[val epoch{n:04d}]\tloss={loss_v:.10f}\n{map_op_rule_v}\n{map_rule_var_v}')
